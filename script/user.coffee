...
timeDay = require('./timeDay.coffee')
...
api.wrap = (user, main=true) ->
  ...
  if main
    user.ops =
      ...
      addTask: (req, cb) ->
        task = misc.taskDefaults(req.body)
  ...
  user.fns =
    ...
    randomDrop: (modifiers) ->
      ...
      chance = scoring.diminishingReturns(chance, 0.75)
      ...
      return if (timeDay.daysSince(user.items.lastDrop.date, user.preferences) is 0) and (user.items.lastDrop.count >= 5 + Math.floor(user._statsComputed.per / 25))
    ...
    updateStats: (stats) ->
      ...
      tnl = scoring.tnl(user.stats.lvl)
    ...
    cron: (options={}) ->
      ...
      daysMissed = timeDay.daysSince user.lastCron, _.defaults({now}, user.preferences)
        ...
        unless completed
          ...
          if (type is 'daily') and repeat
            ...
            _.times daysMissed, (n) ->
              ...
              scheduleMisses++ if timeDay.shouldDo(thatDay, repeat, user.preferences)
          if scheduleMisses > 0
    ...
    preenUserHistory: (minHistLen = 7) ->
      _.each user.habits.concat(user.dailys), (task) ->
        task.history = scoring.preenHistory(task.history) if task.history?.length > minHistLen
        true

      _.defaults user.history, {todos:[], exp: []}
      user.history.exp = scoring.preenHistory(user.history.exp) if user.history.exp.length > minHistLen
      user.history.todos = scoring.preenHistory(user.history.todos) if user.history.todos.length > minHistLen
      #user.markModified? 'history'
